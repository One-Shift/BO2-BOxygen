    ...
    <body>
        ...
        if (isset($_REQUEST['mdl']) && isset($_REQUEST['i']) && !empty($_REQUEST['mdl']) && !empty($_REQUEST['i'])) {
            ...
            if (isset($_COOKIE[$configuration['cookie']])) {
                print
                        '<div id="toolbar">' .
                        '<button onclick="if ($(\'input[type=radio]:checked\').val() != null) {if (confirm(\'Are You Sure?\')) {var code = $(\'input[type=radio]:checked\').val().split(\'.\'); goTo(\'' . $_SERVER["REQUEST_URI"] . '&tp=\' + code[1] + \'&vl=\' + code[0]);}} else {alert(\'Seleccione um ficheiro!\'); return false;}"><i class="fa fa-times"></i> Delete</button>' .
                        '<button onclick="goTo(\'./upload_list.php?mdl=' . $module . '&i=' . $id . '\');"><i class="fa fa-refresh"></i> Atualizar</button>' .
                        '</div>';
